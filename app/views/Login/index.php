<!DOCTYPE html>

<head>
    <link href="../../public/css/styles.css" rel="stylesheet">
    <title>Bonekify</title>
</head>

<body class ="center">
    <div id="register-page">
    <img src = "../../public/img/bonekify.png" id ="logo">
        <h4>Untuk melanjutkan, masuk ke Spotify.</h4>
        <hr>
    </div> 
    <div>
  <form id = "register-form">
    <label for="email-nama">Alamat email atau nama pengguna</label>
    <input type="text" id="email-nama" name="email-nama" placeholder="Alamat email atau nama pengguna">

    <label for="sandi">Kata sandi</label>
    <input type="text" id="sandi" name="sandi" placeholder="Kata sandi">
  
    <input type="submit" value="Masuk">
  </form>
  <p> <strong>Belum punya akun?</strong> <a id = "gantiwarna" href="http://localhost:8080/public/Register">Daftar.</a>
</div>
</body>
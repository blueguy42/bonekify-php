...
<table id='list-lagu' class="col-8 col-s-6" style="font-size:20px; color:black; font-weight:bold; padding:30px">
  ...
  <?foreach($data["penyanyi"]["data"] as $penyanyi){?>
      ...
      <? } if(!isset($data["status"][$penyanyi['user_id']])){?>
      <td  style="border-bottom: 1px groove #ddd;"><button class="subsbutton" style="background-color: RGBA(150,20,20,0.25);border: 1px solid green; border-radius:5px;" onclick="location.href='<?=BASEURL?>/subscribe/add/<?=$penyanyi['user_id']?>/<?=$_SESSION['user_id']?>'" type="button"> SUBSCRIBE </button></td>
  <?}else{?>
    <?if($data["status"][$penyanyi['user_id']]=="ACCEPTED"){?>
        ...
        <?}else{?>
          <td  style="border-bottom: 1px groove #ddd;"><button class="subsbutton" style="background-color: RGBA(20,20,20,0.7);border: 1px solid green; border-radius:5px;" onclick="location.href='<?=BASEURL?>/subscribe/add/<?=$penyanyi['user_id']?>/<?=$_SESSION['user_id']?>'" type="button" disabled> Listen Songs </button></td>  <? }}}?>
</table>
